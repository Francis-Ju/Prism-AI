...
import { Send, FileText, Sparkles, User, Loader2, Plus, Menu, ScanEye, LayoutTemplate, ChevronDown, Zap, BrainCircuit } from 'lucide-react';
...
const ChatSidebar: React.FC<ChatSidebarProps> = ({ 
  ...
  // Refactored container classes to ensure flex-col matches in both modes for sticky footer
  const containerClasses = isCentered 
    ? "flex flex-col h-full max-w-3xl w-full mx-auto transition-all duration-500 ease-in-out relative" 
  ...
  const messageListClasses = isCentered
    ? "flex-1 overflow-y-auto p-4 md:p-8 space-y-8 scrollbar-hide w-full"
    : "flex-1 overflow-y-auto p-4 space-y-6 w-full";

  return (
    <div className={containerClasses}>
      {/* Header / Navigation */}
      <div className={`flex-none p-5 flex items-center justify-between z-20 ${!isCentered ? 'bg-dark-950/80 backdrop-blur-md border-b border-dark-800' : ''}`}>
         <div className="flex items-center gap-3">
            ...
            {!isCentered && (
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-display font-bold text-white tracking-wide text-lg">Prism</h2>
              </div>
            )}
         </div>
      </div>
      ...
      {isCentered && messages.length <= 1 && (
        <div className="absolute top-[15%] left-0 w-full px-6 flex flex-col items-center justify-center space-y-6 animation-fade-in z-0 pointer-events-none">
      ...
      {/* Sticky Input Area - Using flex-none to ensure it doesn't shrink and stays at bottom of flex container */}
